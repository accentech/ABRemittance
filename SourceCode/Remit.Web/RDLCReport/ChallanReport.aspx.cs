    ...
    public partial class ChallanReport : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    delvOption = Request.QueryString["deliverOptionType"].ToUpper();
                    if (delvOption == "COMPANY")
                    {
                        delvOption = "DELIVERY BY COMPANY";
                    } if (delvOption == "CUSTOMER")
                    {
                        delvOption = "DELIVERY BY CUSTOMER";
                    }
                }
                ...
                using (var _context = new ApplicationEntities())
                {
                    ...
                    ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/RDLCReport/Challan.rdlc");
                    ReportViewer1.LocalReport.DataSources.Clear();

                    var query =

                        "SELECT FGItem.Code, FGSize.Size, FGGrade.Grade, FGSalesDeliveryDetail.Lot, FGItem.PackageToSalesRatio as PcsPerCartoon," +
                        " FGSalesDeliveryDetail.DeliveryQuantityInSFT, FGSalesDeliveryDetail.DeliveryQuantityInCTN, " +
                        "FGSalesDelivery.DeliveryChallanNo, FGSalesDelivery.InvoiceNo, FGSales.InvoiceDate,  FGSalesDelivery.DeliveryDate, " +
                        "FGSalesDelivery.CustomerType, FGSalesDelivery.DealerId, FGSalesDelivery.Name, FGSalesDelivery.ContactPersonName, " +
                        "FGSalesDelivery.ContactPhone, FGSalesDelivery.Address, FGSalesDelivery.DeliverySite, FGSalesDelivery.DeliveryOption," +
                        "FGItem.SalesUnitId, FGUOM.UnitName AS UnitName " +
                        "FROM  FGSales "+ 
                    " INNER JOIN FGSalesDelivery "+
                        " ON FGSales.InvoiceNo = FGSalesDelivery.InvoiceNo INNER JOIN FGSalesDeliveryDetail ON FGSalesDelivery.DeliveryChallanNo = FGSalesDeliveryDetail.DeliveryChallanNo " +
                        "INNER JOIN FGItem ON FGSalesDeliveryDetail.FGItemId = FGItem.Id " +
                        "INNER JOIN FGSize ON FGSalesDeliveryDetail.FGSizeId = FGSize.Id AND FGItem.SizeId = FGSize.Id " +
                        "INNER JOIN FGGrade ON FGSalesDeliveryDetail.FGGradeId = FGGrade.Id LEFT JOIN FGUOM ON FGItem.SalesUnitId = FGUOM.Id " +
